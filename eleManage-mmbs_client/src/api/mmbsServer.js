...
export default {
    save(collectionName, obj) {
        ...
        return new Promise((resolve, reject) => {
            classObj.save(obj, {
                success(model) {
                    resolve(resolve({ result: model, success: true }));
                },
                error(model, error) {
    ...
    queryData(pageParams = { collectionName, page: 1, rows: pageSize, param: {}, order: '_created_at' }) {
    ...
    getTreeDataFromCloud(collectionName) {
        try {
            const data = Mmbs.Cloud.run('getTreeData', { collectionName });
            return data;
        } catch (error) {
            return error;
        }
    ...
    login(data) {
        return new Promise((resolve, reject) => {
            Mmbs.User.logIn(data.username, data.password, {
                success: user => {
                    resolve({ result: user, success: true });
                },
                error: (user, error) => {
