...
<template>
	<div v-clickoutside="closeOptions">
		<div @click="toggleOptions">
			<el-input :props="comboProps" v-model="selectedLabel" :icon="comboIcon" placeholder="请选择" readonly="readonly" type="text" />
		</div>
		<div class="el-scrollbar" v-show="showOptions" :style="{'position':'absolute','z-index':'10','overflow': 'auto','width':'100%',height: panelHeight || 'auto'}">
			<el-tree ref="tree" :check-strictly="checkStrictly" default-expand-all :node-key="comboProps.value" :show-checkbox="multiple" :current-node-key="multiple ? '': this.value" :highlight-current="!multiple" :data="treeData" :props="dataProps" @check-change="checkChange" @current-change="handleNodeClick"></el-tree>
		</div>
	</div>
</template>
	...
	methods: {
		setSelected() {
			if (this.multiple && this.defaultData && this.defaultData.length > 0) {
				...
				const node = this.getNodeDataBykey(this.treeData, this.value);
		...
		getNodeDataBykey(data, key) {
			for (let index in data) {
				...
				if (item[this.comboProps.value] == key) {
					return item;
				} else if (item[this.dataProps.children] && item[this.dataProps.children].length) {
					let res = this.getNodeDataBykey(item[this.dataProps.children], key);
					if (res) {
						return res;
					}
				}
