<template>
    <div ref="mapdom" :class="className" :id="domId" style="position: absolute; left: 0px; top: 0px; width: 100%; height: 100%;" param="tileUrl:googleSate;bounds:67.5 0 157.5 67.5;maptype:google;backgroundColor:#F5F3F0;zoom:4;tileStretchPixel:2;sliceZoom:5;maxZoom:20;minZoom:1;fullZoom:20;mapTilesWidth:2048;mapTilesHeight:1536;errorImg:/static/js/webgis/mxImg/blank.png;mapTilesExtendPixel:300"></div>
</template>
<script>
import $ from 'jquery'
...
export default {
	...
	methods: {
		init() {
			this.mxLib = ytMapApi;
			let	params = this.mapOption;
			this.map = new mxLib.Map(this.domId, params);
			/* 开启调试 */
			// map.debug(true);
			/* 设置中心点 */
			...
			if (params.control.enabled) {
				// 地图缩放控件
				const ctl = new mxLib.NavigationControl();
				ctl.setOffset(new mxLib.Size(10, 50));
				ctl.setAnchor(params.control.position != null ? params.control.position : MX_ANCHOR_BOTTOM_RIGHT);
		...
		bindPointToMap(x, y, img, lableName, styleOptions) {
			...
			// 添加点
			const marker = this.addMarker(x, y, img, 40, 40);

			// 添加lable
			const lable = this.addLable(
				point,
				lableName,
		...
		addMarker(x, y, icon, heigth, width) {
			const myIcon = this.markerIcon(icon, heigth, width);
			const point = new this.mxLib.Point(x, y);
			const marker = new this.mxLib.Marker(point, { icon: myIcon }); // 创建标注
		...
		addLable(position, lableName, labelStyle, offset, minZoom, maxZoom) {
			let px = 0;
			const llength = lableName.length / 2;
			let radix = 16;
			...
			let label = new this.mxLib.Label(lableName, opts);
		...
		addPolygon(spoints, options, autoHide) {
			...
			let polygon = new this.mxLib.Polygon(points, options); // 创建多边形
			...
			if (autoHide) {
				...
				polygon.addEventListener('onmouseout', e => {
					if (e.currentTarget) {
