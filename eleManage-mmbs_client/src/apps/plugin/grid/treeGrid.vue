<template>
    <div :style="{width:tableWidth}" class='autoTbale'>
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th v-for="(column,index) in cloneColumns" :key="index" :style="tdStyle(column.width, column.type === 'selection' ? 'center' : column.align)">
            ...
            <tbody>
                <tr v-for="(item,index) in initItems" v-show="show(item) && reload" :key="index" :class="{'child-tr':item.parent, 'selected': item._selected}" @click="RowClick(item, index)">
                    <td v-for="column in columns" :key="column.text" :style="tdStyle(column.width, column.align)">
                        ...
                        <div v-if="column.type === 'action'">
                            <el-button :type="action.type" size="small" @click="RowActionClick(item,$event,index)" v-for='action in (column.actions)' :key="action">{{action.text}}</el-button>
                        </div>
...
export default {
	...
	methods: {
		...
		initData(items, level, parent) {
			// this.initItems = []
			/* 添加空格 */
			const spaceHtml = this.getSpaceHtml(level);
		...
		open(index, item) {
			if (item.children) {
				this.reload = false;
				const self = this;
				this.initItems.forEach((firstLelItem, fIndex) => {
		...
		setChecked(chkIds, isCheck) {
			...
			if (isCheck == null || isCheck == true) {
				...
				for (const k in chkIds) {
					const id = chkIds[k];
					const index = this.checkGroup.findIndex(item => {
						return item == id;
					});
					if (index >= 0) {
